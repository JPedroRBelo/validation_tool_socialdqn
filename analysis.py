...
def main():
	...
	deviation = np.std( np.array(df_array), axis=0 )
	df_scores = pd.DataFrame(matrices,columns = ["Accuracy","Prec.","Recall","Fscore",],index=["Wait","Look","Wave","Handshake","General"])
	#df_deviation = pd.DataFrame(deviation,columns = ["Accuracy","Prec.","Recall","Fscore",],index=["Wait","Look","Wave","Handshake","General"])
	print(standardize(df_scores))
	#print(standardize(df_deviation))
	print('\n*****************************')
